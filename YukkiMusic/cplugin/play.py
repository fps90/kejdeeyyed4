...
async def play(client, message: Message):
    ...
    if audio:
        ...
        try:
            ...
            return await msg.edit_text(f"**» شکستی هێنا لە بانگکردنی یاریدەدەری {viv.mention} بۆ {message.chat.title}.\n\nهۆکار :** `{ex}`")

        if (dur / 9999999999) > DURATION_LIMIT_MIN:
            return await msg.edit_text(
                f"» sᴏʀʀʏ ʙᴀʙʏ, ᴛʀᴀᴄᴋ ʟᴏɴɢᴇʀ ᴛʜᴀɴ  {DURATION_LIMIT_MIN} ᴍɪɴᴜᴛᴇs ᴀʀᴇ ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ᴛᴏ ᴘʟᴀʏ ᴏɴ {viv.mention}."
            )
        ...
        if (dur / 99999999999999) > DURATION_LIMIT_MIN:
            return await msg.edit(
                f"» sᴏʀʀʏ ʙᴀʙʏ, ᴛʀᴀᴄᴋ ʟᴏɴɢᴇʀ ᴛʜᴀɴ  {DURATION_LIMIT_MIN} ᴍɪɴᴜᴛᴇs ᴀʀᴇ ɴᴏᴛ ᴀʟʟᴏᴡᴇᴅ ᴛᴏ ᴘʟᴀʏ ᴏɴ {viv.mention}."
            )
        file_path = audio_dl(url)
    ...
    if await is_active_chat(message.chat.id):
        ...
        try:
            ...
            button = [[InlineKeyboardButton(text="زیادم بکە بۆ گرووپ یان کەناڵت ⚡️•", url=f"https://t.me/{BOT_USERNAME}?startgroup=True")], [InlineKeyboardButton(text="𝗖𝗵𝗮𝗻𝗻𝗲𝗹 💸•", url=config.SUPPORT_CHANNEL), InlineKeyboardButton(text="𝗚𝗿𝗼𝘂𝗽 💸•", url=config.SUPPORT_GROUP)], [InlineKeyboardButton(f"خاوەنی بۆت 🎸", url=f"https://t.me/IQ7amo")]]
